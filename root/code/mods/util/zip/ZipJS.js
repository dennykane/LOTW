...
Using Core.fs_url is deprecated because scripts from there don't have a valid mime type.

But using files from /root/code/... means that we have to be online, unless they are
cached by a serviceWorker. This is an issue with importScripts. If we can rather do this
stuff via blob, then that should solve stuff, but it doesn't seem to want to work.

If we can convert this to not use workers, then that might solve all the problems!!!
...
/*
	var DEFAULT_WORKER_SCRIPTS = {
		deflater: ['z-worker.js', DEFLATE_URL],
		inflater: ['z-worker.js', INFLATE_URL]
	};
*/
...
//		var worker = new Worker(scripts[0]);
		var blob = new Blob([worker_str], {type: 'application/javascript'});
